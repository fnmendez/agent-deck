...
| I10 | A mishearing cannot act on its own | The standing rule — restate and confirm before anything irreversible or outward-facing — lives in the conductor's **standing instructions** (`POLICY.md` Core Rule 7, written from `conductorPolicyTemplate`), not in the per-message preamble: a freshly started or cycled session must still have it, and a preamble is the wrong home for a standing rule. Each side asserts the other, so the rule cannot vanish in the gap between the two files: `internal/session` tests that the template carries it, and `tests/test_voice_prompt.py` tests that the template still exists and says it. Below `MIN_CONFIDENCE` the note is shown but never executed. A redelivered note is refused (durable ledger keyed by Telegram's `file_unique_id`, content hash when Telegram omits one). The ledger guards *execution* only, so re-forwarding a third party's note is not refused — nothing is being run. |
